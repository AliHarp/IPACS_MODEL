...
# AMY: Depends on input_filename from ipacs_main_script.R
...
# AMY: Currently depends on prior scripts - need to change
...
scenario_table<- cbind(c(1:length(levels(factor(p1_bristol_l$scenario)))),levels(factor(p1_bristol_l$scenario)))
new_table<-str_split(levels(factor(p1_bristol_l$scenario)),"_",simplify = TRUE)
scenario_table<-cbind(c(1:length(levels(factor(p1_bristol_l$scenario)))),new_table)

scenario_table<-gsub('Cap','',scenario_table)
scenario_table<-gsub('los','',scenario_table)
scenario_table<-gsub('Arr','',scenario_table)

#replace(scenario_table,"Baseline","Specification")
colnames(scenario_table)<-c("Scenario #","Capacity","Length of Stay","Arrival")
scenario_table<-as.data.frame(scenario_table)
scenario_table[scenario_table == 'B'] <- 'Baseline'#p1_bristol_l <- bind_rows(P1_B_previous, p1_bristol_l)
